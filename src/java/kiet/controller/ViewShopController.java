...
public class ViewShopController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<ItemDTO> list = dao.getProduct();
            request.setAttribute("PRODUCT", list);
            
        } catch (NamingException e) {
            log("NamingException ", e.getCause());
        } catch (SQLException e) {
            log("SQLError ", e.getCause());
        } finally {
            request.getRequestDispatcher("onlineShop.jsp").forward(request, response);
        }
    }
